...
class Memory2(nn.Module):
    ...
    def __init__(self, memory_units, memory_unit_size,device=None):
    ...
    def loading(self):
        loading=torch.where(self.memory>1e-6,1,0)
        loading=torch.sum(loading,dim=-1)
        print(loading)
        return torch.count_nonzero(loading)/self.n
